...
| Embeddings + grounded answers | **Vertex AI** `text-embedding-004` + **Gemini** |
...
flowchart LR
  ...
  subgraph query[Query]
    Q2[User question] --> EQ[Vertex query embedding]
...
2. Copy `.env.example` to `.env` and set `GCP_PROJECT` (Vertex uses Application Default Credentials or attached service account).
...
- `GET /health` — Qdrant + Vertex + reranker checks
...
Terraform lives in **`deploy/terraform/`** (`terraform.tfvars.example` → `terraform.tfvars`). It provisions a **Cloud Run v2** service with **`roles/aiplatform.user`**, injects **`QDRANT_API_KEY`** from Secret Manager, sets **`PRODUCTION_MODE=true`** (blocks in-memory Qdrant), and disables **`/docs`** by default.
...
**Regression (pytest, mocked RAG — no Vertex/Qdrant billable calls on `/v1/*`):**
...
Add `--hit-query` only against a disposable environment (each call runs **Vertex** + **Qdrant**).
...
- Set `EMBEDDING_DIMENSIONS` to match your embedding model (768 for `text-embedding-004`).
